...
- [x] Translation proxy endpoints：用 Ditto provider adapters 实现“OpenAI in/out”的 `POST /v1/responses` + `POST /v1/chat/completions` + `POST /v1/embeddings`（feature `gateway-translation`；不依赖上游 OpenAI-compatible 服务）
