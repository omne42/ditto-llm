...
- Gateway translation proxy: OpenAI-compatible `/v1/chat/completions`, `/v1/responses`, `/v1/embeddings`, `/v1/moderations`, and `/v1/images/generations` backed by Ditto providers (feature `gateway-translation`).
...
- The default build is not an API gateway/proxy; the `gateway` feature adds a lightweight control-plane + HTTP service. The `gateway-translation` feature adds translation for `/v1/chat/completions`, `/v1/responses`, `/v1/embeddings`, `/v1/moderations`, and `/v1/images/generations`. Full OpenAI surface translation (audio/batches/etc) is tracked in `TODO.md`.
...
- OpenAI-compatible translation (feature `gateway-translation`): `POST /v1/chat/completions`, `POST /v1/responses`, `POST /v1/embeddings`, `POST /v1/moderations`, and `POST /v1/images/generations` can be served by a backend with `provider` configured (adds `x-ditto-translation: <provider>`).
