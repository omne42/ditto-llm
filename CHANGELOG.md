...
- Runtime/CLI: move data-root discovery, CLI flag probing, directory bootstrap, and default-file materialization into `ditto-core::resources`; keep `ditto-server::data_root` focused on Ditto-owned default filenames/templates and server path layout.
- Security: bump `rustls-webpki` to `0.103.13` to address `RUSTSEC-2026-0104` in the locked dependency graph.
...
- Performance: simplify `StreamCollector` tool-call tracking by removing the extra `seen_tool_call_ids` set and consolidating to single-map state, reducing hash lookups and per-request memory overhead in streaming hot paths.
- Performance: add `front/back` fast-path removal in SDK and Gateway in-memory LRU queues to avoid unnecessary full `VecDeque` scans for common recency-update/eviction patterns.
...
- SDK: `CacheLayer` now throttles full TTL-prune scans on read paths to once per second while still enforcing exact per-key expiry checks (reduces O(n) cache-scan overhead under read-heavy traffic).
