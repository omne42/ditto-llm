...
- Agent: built-in tool wrappers and executors (`http_fetch`, `fs_read_file`, `fs_write_file`, `fs_list_dir`, `shell_exec`) for `ToolLoopAgent` (feature `agent`).
...
- Gateway: config supports `${ENV_VAR}` interpolation in proxy backend `base_url`/`headers`/`query_params` and `virtual_keys[].token` (resolved at startup).
