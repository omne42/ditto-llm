...
- `src/catalog/` 的静态能力目录边界是准确的
- `src/runtime/` 与 `runtime_registry` 作为动态装配层是成立的
- `src/config/`、`src/providers/`、`src/foundation/`、`src/llm_core/` 的职责分离总体正确

审查模型给出的主要问题有三条：

1. `provider_transport` 与 `session_transport` 没有作为显式根边界存在
2. `provider_options` 被埋在 `src/types/` 里，边界不够显性
...
- `src/provider_transport/mod.rs`
- `src/provider_transport/config.rs`
- `src/provider_transport/http.rs`

调整：

- 原 `src/config/http.rs` 已迁入 `src/provider_transport/config.rs`
- 原 `src/utils/http.rs` 已迁入 `src/provider_transport/http.rs`
...
- `src/session_transport/mod.rs`
- `src/session_transport/sse.rs`
- `src/session_transport/streaming.rs`

调整：

- 原 `src/utils/sse.rs` 已迁入 `src/session_transport/sse.rs`
- 原 `src/utils/streaming.rs` 已迁入 `src/session_transport/streaming.rs`
...
- `src/provider_options/mod.rs`
- `src/provider_options/envelope.rs`
- `src/provider_options/support.rs`

调整：

- 原 `src/types/provider_options_envelope.rs` 已迁入 `src/provider_options/envelope.rs`
- 原 `src/types/provider_options_support.rs` 已迁入 `src/provider_options/support.rs`
- `ProviderOptions`、`ReasoningEffort`、`ReasoningSummary`、`ResponseFormat`、`JsonSchemaFormat` 已从 `src/types/llm.rs` 提升到 `src/provider_options/mod.rs`
...
- 新增 `src/runtime_registry/mod.rs`
- 原 `src/runtime/registry.rs` 已迁入 `src/runtime_registry/snapshot.rs`
- 原 `src/runtime/provider_catalog.rs` 已迁入 `src/runtime_registry/catalog.rs`
...
- `src/runtime/route.rs`
- `src/runtime/explain.rs`
- `src/runtime/transport.rs`
- `src/catalog/mod.rs`
- `src/lib.rs`
- `src/runtime_registry/mod.rs`
...
- `src/runtime_registry/catalog.rs` 现在只保留 frontdoor 类型与共享数据结构
- 新增 `src/runtime_registry/queries.rs`
- 新增 `src/runtime_registry/semantics.rs`
...
- 新增 `src/runtime/route_selection.rs`
- 新增 `src/runtime/route_endpoint.rs`
- `src/runtime/route.rs` 保留 explain plan、前门入口和 join 过程
...
- `src/providers/openai/text.rs` 的 `from_config()` 现在默认注入 builtin catalog surface resolver
...
- 删除 `src/providers/openai_compatible_family.rs`
- 新增 `src/providers/openai_compat_profile.rs`
...
- `src/capabilities/audio.rs`
- `src/capabilities/batch.rs`
- `src/capabilities/image.rs`
- `src/capabilities/image_edit.rs`
- `src/capabilities/moderation.rs`
- `src/capabilities/rerank.rs`
- `src/capabilities/video.rs`
...
- 将 `catalog/resolver.rs` 物理迁入 `src/runtime/route.rs`
...
- `src/lib.rs`
- `src/runtime/mod.rs`
- `src/runtime/model_builders.rs`
- `src/catalog/mod.rs`
- `src/runtime_registry/mod.rs`
- `src/provider_transport/mod.rs`
- `src/provider_options/mod.rs`
- `src/session_transport/mod.rs`
- `src/gateway/application/translation/mod.rs`
- `src/providers/openai_compatible/client.rs`
...
- `src/runtime/resolver.rs` 不再从 root 导入 `OperationKind` / `RuntimeRoute*` / `Result`
- `src/runtime/explain.rs`、`src/runtime/transport.rs`、`src/runtime/route.rs`、
  `src/runtime/model_builders.rs` 改为直接依赖 `contracts` 与
  `foundation::error`
- `src/provider_transport/config.rs`、`src/provider_transport/http.rs`、
  `src/session_transport/sse.rs`、`src/session_transport/streaming.rs` 不再依赖
  root `DittoError` / `Result`
- `src/provider_options/envelope.rs`、`src/provider_options/support.rs` 改为
  `super::*`，不再反向引用 `crate::provider_options::*`
- `src/providers/openai/chat_completions.rs` 改为直接依赖
...
- `src/runtime/model_builders.rs` 里的 provider 构造入口，已从
...
- `src/providers/openai_compatible/client.rs` 不再在请求构造逻辑里直接调用
  `std::env::var`
...
`src/gateway/application/translation/mod.rs` 的物理拆分这条意见这轮没有采纳。
...
- [src/llm_core/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/llm_core/mod.rs)
  删除了 `pub mod error`
...
- `src/gateway/application/translation/mod.rs` 不再直接导入
  `builtin_runtime_registry_catalog`
- `src/runtime/model_builders.rs` 新增：
  - `builtin_runtime_supports_capability(...)`
  - `builtin_runtime_supports_file_builder(...)`
- `src/runtime/mod.rs` 将上述两个入口收进 runtime 前门
...
- 新增 `src/gateway/adapters/cache/local_lru.rs`
- 原先写在 `src/gateway/application/translation/mod.rs` 里的 `ModelCache<V>`
  已移除
...
- `src/contracts/ids.rs` 新增
  `invocation_operations_for_capability(...)`
- `src/contracts/mod.rs` 公开该契约 helper
- `src/runtime/model_builders.rs` 删除本地
...
- [src/lib.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/lib.rs)
  移除了 `#[cfg(test)] mod compat;`，不再把 `compat` 放进 crate 编译图
- 直接删除了整个
  [src/compat](/root/autodl-tmp/zjj/p/ditto-llm/src/compat)
...
- [src/config/auth.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/config/auth.rs)
- [src/config/env.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/config/env.rs)
- [src/config/provider_config.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/config/provider_config.rs)
- [src/provider_transport/config.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/provider_transport/config.rs)
- [src/providers/openai_compat_profile.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai_compat_profile.rs)
...
- [src/providers/openai/text.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/text.rs)
...
- [src/catalog/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/catalog/mod.rs)
...
- [src/runtime/resolver.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/resolver.rs)
...
- [src/runtime/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/mod.rs)
  不再公开：
...
- [src/runtime/explain.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/explain.rs)
  的 `explain_runtime_route(...)` 改为 `pub(crate)`
- [src/runtime/transport.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/transport.rs)
...
- [src/runtime/model_builders.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/model_builders.rs)
  在构建 OpenAI text model 时注入静态支持矩阵查询
- 同处补了显式标记注释：
  `MODEL-SURFACE-RESOLUTION`
- [src/providers/openai_compatible/client.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai_compatible/client.rs)
...
- `src/runtime_registry/catalog.rs` 仍然暴露一组直接绑死
...
- `src/runtime_registry/catalog.rs` 新增
...
- `src/runtime/model_builders.rs`
- `src/providers/openai/text.rs`
- `src/gateway/application/translation/mod.rs`
- `src/config_editing.rs`
...
2. `src/foundation/mod.rs` 仍然通过 `foundation::env` / `foundation::utils`
   二次代持 `config` / `utils`
...
`src/gateway/application/translation/mod.rs` 里仍然存在一些 crate-root 紧凑导入，
...
- builder 逻辑实际在 `src/runtime/model_builders.rs`
...
- `src/bin/ditto_gateway/config_cli.rs`
...
- runtime builder 不再按 `ProviderConfig.default_model` 预先塞死布尔开关；
  改为在 [src/runtime/model_builders.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/model_builders.rs)
  注入一个按模型查询的 resolver
...
- `src/runtime_registry/catalog.rs` 新增 crate-private helper：
...
- `src/runtime_registry/mod.rs` 统一 re-export 这些 helper 给 crate 内部使用
- `src/providers/openai/text.rs` 不再直连 `catalog`，改为通过
  `runtime_registry::builtin_provider_supports_operation(...)` 判断 text surface
- `src/providers/openai_compatible/client.rs` 不再读取 provider behavior descriptor，
...
- [src/runtime_registry/catalog.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime_registry/catalog.rs)
...
- [src/gateway/application/translation/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/gateway/application/translation/mod.rs)
...
- `src/providers/openai_compatible/client.rs` 不再 import `runtime_registry`
...
- `src/runtime/model_builders.rs` 在构建 openai-compatible LLM 时，通过
...
- `src/runtime/model_builders.rs` 删除 `canonical_builder_provider_from_hint(...)`
- builder 选择现在统一通过
...
- 删除 `src/config/provider_config.rs` 里对 `RuntimeRouteRequest` 的 inherent impl
...
1. `src/config_editing.rs` 仍然在配置编辑层直接发 HTTP 做模型发现，破坏了
   `config_editing` 作为确定性文档编辑层的边界
2. `src/runtime/model_builders.rs` 仍然在 builder 内部自行查 `builtin_registry()` 并重复推导装配策略，
   没有完全收敛到上游显式 runtime 计划
3. `src/providers/openai_compatible/client.rs` 仍然在 provider 侧直接读 `Env`
   控制请求开关，破坏了显式契约传导

非阻塞意见里，Omne 还点到了：

- `src/capabilities/text.rs` 还在通过 `pub use crate::types::*` 形成第二 owner
...
- `src/config_editing.rs` 删除了基于 `reqwest` 的 `discover_models_for_provider()` 及相关 HTTP 解析逻辑
...
- `src/bin/ditto_gateway/config_cli.rs` 现在负责 `--discover-models` 的 HTTP 发现，
...
- `src/config/provider_config.rs` 新增 `OpenAiCompatibleConfig`
- `ProviderConfig` 新增 `openai_compatible: Option<OpenAiCompatibleConfig>`
- `src/providers/openai_compatible/client.rs` 不再扫描
  `OMNE_OPENAI_COMPAT_SEND_PROMPT_CACHE_KEY` /
  `OMNE_OPENAI_COMPAT_SEND_TOOL_CALL_THOUGHT_SIGNATURE`
  这类环境变量
...
- `src/capabilities/text.rs` 把 `pub use crate::types::{...}` 改成了私有 `use`
...
- 但 `src/capabilities/text.rs` 仍然 `pub use crate::provider_options::{...}`
...
- [src/capabilities/text.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/capabilities/text.rs)
...
- `src/config/editing.rs` 把文件变更、交互式编辑、模型探测这类副作用流程挂在
  `config` 纯配置层命名空间下面，破坏了 L0 `config` 作为无状态配置语义 owner 的边界
- `src/gateway/application/translation/mod.rs` 通过
...
- `src/config/editing.rs` 已移动为 `src/config_editing.rs`
- `src/config/mod.rs` 不再声明 `pub mod editing;`
- `src/bin/ditto_gateway/config_cli.rs`
- `src/compat/profile/mod.rs`
...
- `src/gateway/application/translation/mod.rs` 移除了
...
3. `src/runtime/model_builders.rs` 里仍有 builtin singleton lookup，后续可考虑 injection
...
- [src/lib.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/lib.rs) 移除了
  `foundation::error` 与 `llm_core` 的 `pub(crate)` alias
- 在 [src/lib.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/lib.rs) 增加了显式标识
...
- 新增 [src/runtime/builtin.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/builtin.rs)
...
- 新增 [src/contracts/outcome.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/contracts/outcome.rs)
- 新增 [src/contracts/tool_call.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/contracts/tool_call.rs)
- 增加显式标识 `CONTRACTS-OUTCOME-OWNER`
- [src/contracts/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/contracts/mod.rs)
  现在公开：
  - `FinishReason`
  - `Usage`
  - `Warning`
  - `parse_tool_call_arguments_json_or_string`（crate 内部）
- [src/types/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/types/mod.rs)
  增加显式标识 `TYPES-PAYLOAD-ONLY`
- [src/types/llm.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/types/llm.rs)
  不再定义这三类 outcome contract，只引用 `crate::contracts::*`
- 删除 [src/types/tool_call.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/types/tool_call.rs)
...
- 新增 [src/contracts/llm.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/contracts/llm.rs)
...
- [src/llm_core/model.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/llm_core/model.rs)
- [src/llm_core/layer.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/llm_core/layer.rs)
- [src/llm_core/stream.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/llm_core/stream.rs)
- [src/capabilities/text.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/capabilities/text.rs)
- [src/session_transport/streaming.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/session_transport/streaming.rs)
- [src/object/core.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/object/core.rs)
  这几处核心调用路径已改成显式依赖 `crate::contracts::*`

#### 2. 把 request-scoped provider passthrough 行为收回 `provider_options`

- 新增 [src/provider_options/request.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/provider_options/request.rs)
...
- [src/provider_options/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/provider_options/mod.rs)
  现在由 `provider_options` 显式对外暴露这些 helper
- `GenerateRequest` 本体不再携带 provider passthrough merge / parse 逻辑，只保留数据字段
- [src/providers/anthropic/messages_api.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/anthropic/messages_api.rs)
- [src/providers/bedrock/messages_api.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/bedrock/messages_api.rs)
- [src/providers/bedrock/client.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/bedrock/client.rs)
- [src/providers/cohere/chat_api.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/cohere/chat_api.rs)
- [src/providers/google/generate_api.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/google/generate_api.rs)
- [src/providers/openai/chat_completions.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/chat_completions.rs)
- [src/providers/openai/completions_legacy.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/completions_legacy.rs)
- [src/providers/openai/responses.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/responses.rs)
- [src/providers/openai_compatible/chat_completions.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai_compatible/chat_completions.rs)
- [src/providers/vertex.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/vertex.rs)
- [src/gateway/application/translation/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/gateway/application/translation/mod.rs)
...
- [src/capabilities/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/capabilities/mod.rs)
...
- 新增 [src/provider_transport/policy.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/provider_transport/policy.rs)
...
- [src/provider_transport/http.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/provider_transport/http.rs)
...
- 新增 [src/session_transport/policy.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/session_transport/policy.rs)
- 增加显式标识 `SESSION-TRANSPORT-POLICY-OWNER`
- 新增公开类型：
  - `SessionTransportPolicy`
  - `WebsocketBaseUrlResolution`
  - `WebsocketBaseUrlRewrite`
- [src/session_transport/sse.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/session_transport/sse.rs)
...
- [src/runtime/route.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/route.rs)
...
- 删除 [src/types/llm.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/types/llm.rs)
- [src/types/mod.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/types/mod.rs) 不再 `mod llm;`，也不再 re-export：
  - `GenerateRequest`
  - `GenerateResponse`
  - `Message`
  - `ContentPart`
...
- `src/providers/openai/client.rs` 曾直接读取进程环境变量
...
- [src/providers/openai/client.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/client.rs)
  新增显式 override 字段：
...
- [src/providers/openai/raw_responses.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/providers/openai/raw_responses.rs)
...
- [src/runtime/builder_protocol.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/builder_protocol.rs)
...
- [src/runtime/builder_backends.rs](/root/autodl-tmp/zjj/p/ditto-llm/src/runtime/builder_backends.rs)
