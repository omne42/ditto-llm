...
此时顶层 `provider` 负责选择 translation backend 的适配器，`provider_config.provider` 用来把具体 node 绑定到 registry 中的 provider pack；`enabled_capabilities` 则声明这个 node 启用哪些一级 capability。

gateway 会把 OpenAI in/out 翻译为对应 provider 的 native 请求（并尽量保持 OpenAI shape）。
...
- `provider` / `provider_config`：translation backend 配置；其中 `provider_config.provider` 绑定运行时 provider pack，`provider_config.enabled_capabilities` 声明 node 启用的一级 capability（详见「SDK → ProviderConfig 与 Profile」）
...
- `backends[].provider_config.*`（node 级字段，如 provider/enabled_capabilities/base_url/default_model/http_headers/http_query_params/model_whitelist/auth/upstream_api/normalize_to/normalize_endpoint）
- `virtual_keys[].token`
...
## observability：观测、采样与脱敏（重要）

`observability` 定义了 Ditto Gateway 对外输出的观测数据如何处理：

- `observability.redaction`：统一的脱敏规则
- `observability.sampling`：结构化事件输出的采样率

覆盖范围：

- 结构化 JSON logs（`--json-logs`）
- 审计日志与 audit export（sqlite/postgres/mysql/redis store）
- Devtools JSONL（`--devtools`）
- Prometheus labels（`GET /metrics/prometheus`）

> 这不是“安全万无一失”的魔法，而是把泄露面和采样策略收敛到一处可配置的 policy。生产环境仍需要最小权限、日志保留策略与访问控制。
...
- `sampling.json_logs_rate` / `sampling.audit_rate` / `sampling.devtools_rate` 默认都是 `1.0`（即不采样，全部保留）

Prometheus 仍然记录完整指标，但 label 值会应用与 JSON logs / audit / devtools 相同的 redaction 规则。
...
```json
{
  "observability": {
    "redaction": {
      ...
      "redact_key_names": ["token", "api_key", "secret", "virtual_key_id"],
...
- `sampling.*_rate` 必须是 `0.0..=1.0` 的有限浮点数；`0.0` 表示全部丢弃，`1.0` 表示全部保留。
- 采样会尽量按 `request_id` / `trace_id` / `response_id` / `session_id` / `id` 做稳定判定，这样同一请求的多条结构化事件更容易一起保留或一起丢弃。
- Prometheus label 只做 redaction，不做采样，避免把计数/时延指标变成“近似值”。
- 如果你真的要关闭脱敏（不推荐），把所有 `redact_*` 列表设为空数组即可；`replacement` 仍必须是非空字符串。
- `ditto-gateway` 启动时会校验 `redact_json_pointers`、`redact_regexes` 与 `sampling.*_rate`，避免带着错误配置在生产里“以为自己已经保护好了”。
