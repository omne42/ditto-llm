...
- `GET /v1/responses/*`、`GET /v1/responses/*/input_items`、`DELETE /v1/responses/*` 当前走 best-effort local store，只保证读写“同一进程内由 non-stream translation create 生成”的 response，并要求调用方使用该 gateway 返回的 gateway-scoped response id。
