...
- `crates/ditto-core/src/config/provider_config.rs`、`crates/ditto-core/src/config/auth.rs`、`crates/ditto-core/src/config/env.rs`：ProviderConfig / ProviderAuth / Env 等配置语义
- `crates/ditto-core/src/catalog/generated/providers/*.rs`：按 provider 拆分的运行时 Rust 静态目录
- `catalog/provider_models/*.toml`：更细的模型、接口、证据数据
- `crates/ditto-core/src/providers/*`：实际请求与响应编解码逻辑
- `crates/ditto-server/src/config_editing.rs`：交互式配置与 provider/model 操作
...
- 真正参与运行时的，是 `crates/ditto-core/src/catalog/` 下的 Rust 静态定义。
- 如果未来需要自动化同步，应该是“离线脚本 -> 生成 Rust 源文件”，而不是“运行时读 TOML/JSON”。

当前已经补上一层 Rust reference schema/validator：`crates/ditto-core/src/catalog/reference_schema.rs`。
...
- `scripts/generate_rust_provider_catalog.py` 会从 `catalog/provider_models/*.json` 生成按 provider 拆分的 `crates/ditto-core/src/catalog/generated/providers/*.rs` 与聚合入口 `crates/ditto-core/src/catalog/generated/providers/mod.rs`
...
- 拆分前的 `profile/*_model_catalog.rs` / `generated_catalogs.rs` 方案已经退出当前运行时路径；现在以 `crates/ditto-core/src/catalog/generated/providers/*.rs` 作为唯一运行时生成产物
...
这些类型已经接入 `crates/ditto-core/src/catalog/` 与 runtime route resolver，不再只是文档草图。
...
- 新增 `crates/ditto-core/src/catalog/` 模块
- 定义核心 enums / structs
- 不改现有 HTTP provider 行为
- 不强行一次性替换现有 provider runtime 实现
...
1. 文档落地
2. `crates/ditto-core/src/catalog/` 第一版核心类型
