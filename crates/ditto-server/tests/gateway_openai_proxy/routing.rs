...
async fn openai_compat_proxy_retries_retryable_statuses_across_backends() {
    ...
    let config = GatewayConfig {
        ...
        router: RouterConfig {
            default_backends: vec![
                ditto_server::gateway::RouteBackend {
                    backend: "primary".to_string(),
                    weight: 1.0,
                },
                ditto_server::gateway::RouteBackend {
    ...
    let state = GatewayHttpState::new(gateway)
        ...
        .with_proxy_routing(ditto_server::gateway::ProxyRoutingConfig {
            ...
            circuit_breaker: ditto_server::gateway::ProxyCircuitBreakerConfig::default(),
            ..Default::default()
        });
    let app = ditto_server::gateway::http::router(state);
...
async fn openai_compat_proxy_circuit_breaker_skips_unhealthy_backends() {
    ...
    let state = GatewayHttpState::new(gateway)
        ...
        .with_proxy_routing(ditto_server::gateway::ProxyRoutingConfig {
            ...
            circuit_breaker: ditto_server::gateway::ProxyCircuitBreakerConfig {
                enabled: true,
                failure_threshold: 1,
...
#[cfg(feature = "gateway-routing-advanced")]
#[tokio::test]
async fn openai_compat_proxy_fallbacks_on_configured_status_when_retry_disabled() {
    if ditto_core::utils::test_support::should_skip_httpmock() {
        return;
    }
    let primary = MockServer::start();
    ...
    let primary_mock = primary.mock(|when, then| {
        when.method(POST)
            .path("/v1/responses")
            .header("authorization", "Bearer sk-primary")
            .header("x-request-id", "req-fallback-status");
        then.status(503)
            .header("content-type", "application/json")
            .body(r#"{"error":{"message":"temporarily unavailable","type":"server_error"}}"#);
    });
    let secondary_mock = secondary.mock(|when, then| {
        when.method(POST)
            .path("/v1/responses")
...
#[cfg(feature = "gateway-routing-advanced")]
#[tokio::test]
async fn openai_compat_proxy_does_not_fallback_on_non_configured_status_when_retry_disabled() {
    if ditto_core::utils::test_support::should_skip_httpmock() {
        return;
    }
    let primary = MockServer::start();
    ...
    let primary_mock = primary.mock(|when, then| {
        when.method(POST)
            .path("/v1/responses")
            .header("authorization", "Bearer sk-primary")
            .header("x-request-id", "req-no-fallback-status");
        then.status(503)
            .header("content-type", "application/json")
            .body(r#"{"error":{"message":"temporarily unavailable","type":"server_error"}}"#);
    });
...
async fn openai_compat_proxy_health_check_skips_unhealthy_backends() {
    ...
    let state = GatewayHttpState::new(gateway)
        .with_proxy_backends(proxy_backends)
        .with_proxy_routing(ditto_server::gateway::ProxyRoutingConfig {
            health_check: ditto_server::gateway::proxy_routing::ProxyHealthCheckConfig {
                enabled: true,
...
async fn openai_compat_proxy_does_not_fallback_on_network_error_when_disabled() {
    ...
    let state = GatewayHttpState::new(gateway)
        ...
        .with_proxy_routing(ditto_server::gateway::ProxyRoutingConfig {
            retry: ditto_server::gateway::ProxyRetryConfig {
                network_error_action: ditto_server::gateway::proxy_routing::ProxyFailureAction::None,
                max_attempts: Some(2),
                ..Default::default()
            },
            ..Default::default()
