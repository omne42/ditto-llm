...
impl FsToolExecutor {
    pub fn new(root: impl Into<PathBuf>) -> Result<Self> {
        ...
        Ok(Self {
            runtime: OmneFsToolRuntime::new(root, 256 * 1024, 200),
        })
    }

    pub fn with_max_bytes(mut self, max_bytes: usize) -> Self {
        self.runtime = self.runtime.with_max_bytes(max_bytes);
        self
    }

    pub fn with_max_list_entries(mut self, max_list_entries: usize) -> Self {
        self.runtime = self.runtime.with_max_results(max_list_entries);
        self
    }
}
