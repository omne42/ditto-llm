...
fn lock_or_err<'a, T>(mutex: &'a Mutex<T>, context: &str) -> Result<MutexGuard<'a, T>> {
    mutex.lock().map_err(|_| {
        ditto_core::invalid_response!(
            ...
            "message" => format!("{context} lock poisoned")
        )
    })
}
...
impl LanguageModel for StubModel {
    ...
    async fn generate(&self, request: GenerateRequest) -> Result<GenerateResponse> {
        ...
        responses.pop_front().ok_or_else(|| {
            ditto_core::invalid_response!(
                "error_detail.freeform",
                "message" => "stub model has no responses left"
            )
        })
    }
    ...
    async fn stream(&self, _request: GenerateRequest) -> Result<StreamResult> {
        Err(ditto_core::invalid_response!(
            "error_detail.freeform",
            "message" => "stub model does not support stream"
        ))
    }
}
    ...
    match &tool_messages[0].content[0] {
        ...
        _ => {
            return Err(ditto_core::invalid_response!(
                "error_detail.freeform",
                "message" => "expected tool result content"
            ));
        }
    }
...
async fn approval_denied_skips_executor() -> Result<()> {
    ...
    let Some(denied) = denied else {
        return Err(ditto_core::invalid_response!(
            "error_detail.freeform",
            "message" => "missing denied tool result message"
        ));
    };
