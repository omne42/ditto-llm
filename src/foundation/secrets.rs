...
fn secret_command_timeout(env: &dyn SecretEnvironment) -> Duration {
    let ms = env
        .get_secret_env("DITTO_SECRET_COMMAND_TIMEOUT_MS")
    ...
    let secs = env
        .get_secret_env("DITTO_SECRET_COMMAND_TIMEOUT_SECS")
        .and_then(|raw| raw.trim().parse::<u64>().ok())
        .filter(|value| *value > 0)
...
async fn run_secret_command(cmd: &SecretCommand, env: &dyn SecretEnvironment) -> Result<String> {
    ...
    let timeout_error = match tokio::time::timeout(timeout, child.wait()).await {
        Ok(status) => {
            let status = status
                .map_err(|err| DittoError::AuthCommand(format!("wait {}: {err}", cmd.program)))?;
            Ok(status)
        }
        Err(_) => {
