...
use super::provider_config::{ProviderApi, ProviderCapabilities, ProviderConfig};
use crate::catalog::{AuthMethodKind, CapabilityKind};
use crate::runtime::{
    BuiltinProviderPreset, ResolvedProviderConfigSemantics, builtin_models_for_provider,
    builtin_provider_candidates_for_model, builtin_provider_capability_summary,
    builtin_provider_preset, resolve_provider_config_semantics,
};
...
fn apply_builtin_provider_defaults(
    ...
    if clean_option_string(req.base_url.clone()).is_none() {
        if let Some(base_url) =
            builtin_provider_preset(req.name.as_str()).and_then(|preset| preset.default_base_url)
        {
...
fn provider_default_model_prompt_label(provider_name: &str) -> String {
    let examples: Vec<&str> = builtin_models_for_provider(provider_name)
        ...
        .collect();
    let summary = builtin_provider_capability_summary(provider_name);
...
fn resolved_provider_editor_hint(
    ...
    clean_option_string(req.provider.clone()).or_else(|| {
        let provider_name = non_empty_trimmed(&req.name)?;
        builtin_provider_preset(&provider_name)
...
fn resolved_provider_editor_preset(
    ...
    let provider_name = resolved_provider_editor_hint(req, resolved_upstream)?;
    builtin_provider_preset(provider_name.as_str())
}
