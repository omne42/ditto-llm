...
impl ProviderApi {
    ...
    pub const fn runtime_api(self) -> RuntimeProviderApi {
        match self {
            Self::OpenaiChatCompletions => RuntimeProviderApi::OpenaiChatCompletions,
            Self::OpenaiResponses => RuntimeProviderApi::OpenaiResponses,
            Self::GeminiGenerateContent => RuntimeProviderApi::GeminiGenerateContent,
            Self::AnthropicMessages => RuntimeProviderApi::AnthropicMessages,
        }
    }
}
...
impl ProviderConfig {
    pub fn runtime_hints(&self) -> RuntimeProviderHints<'_> {
        RuntimeProviderHints {
            configured_provider: self.provider.as_deref(),
            base_url: self.base_url.as_deref(),
            default_model: self.default_model.as_deref(),
            enabled_capabilities: self.enabled_capabilities.as_slice(),
            http_query_params: (!self.http_query_params.is_empty())
...
pub fn merge_provider_config(
    ...
    {
        base.provider = Some(provider.to_string());
    }
    if !overrides.enabled_capabilities.is_empty() {
        base.enabled_capabilities = normalize_string_list(overrides.enabled_capabilities.clone());
    }
