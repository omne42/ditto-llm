...
use crate::{DittoError, ProviderApi, Result};

use super::catalog_bridge::{
    builtin_models_for_provider, builtin_provider_candidates_for_model, builtin_provider_preset,
};
use crate::catalog::AuthMethodKind;
...
pub async fn upsert_provider_config(req: ProviderUpsertRequest) -> Result<ProviderUpsertReport> {
    let provider_name = non_empty_trimmed(&req.name)
        .ok_or_else(|| DittoError::Config("provider name must not be empty".to_string()))?;
...

    {
        ...
        let base_url = clean_option_string(req.base_url.clone())
            ...
            .or_else(|| infer_default_base_url(req.namespace, &provider_name, resolved_upstream));
        ...
        apply_provider_capabilities_table(provider_table, &req);
    }
    ...
    if req.discover_models {
        let provider_table =
            ensure_table_path(&mut doc, &[namespace, "providers", provider_name.as_str()]);
        let base_url = existing_string_value(provider_table.get("base_url")).ok_or_else(|| {
            DittoError::Config("provider base_url is required for discover_models".to_string())
        })?;

        discovered_models =
            discover_models_for_provider(&req, &base_url, resolved_upstream).await?;
...
pub async fn upsert_model_config(req: ModelUpsertRequest) -> Result<ModelUpsertReport> {
    let model_name = non_empty_trimmed(&req.name)
        .ok_or_else(|| DittoError::Config("model name must not be empty".to_string()))?;
...
pub fn complete_provider_upsert_request_interactive(
    ...
    apply_builtin_provider_defaults(&mut req);
    ...
    let inferred_base = infer_default_base_url(
        req.namespace,
        req.name.as_str(),
        req.upstream_api
            .unwrap_or_else(|| default_api_for_namespace(req.namespace)),
    ...
    let default_model_label = provider_default_model_prompt_label(req.name.as_str());
...
fn ensure_interactive_stdio() -> Result<()> {
    ...
    if !std::io::stdin().is_terminal() || !std::io::stdout().is_terminal() {
        return Err(DittoError::Config(
            "interactive mode requires a TTY".to_string(),
        ));
    }
    Ok(())
}
...
fn apply_builtin_provider_defaults(req: &mut ProviderUpsertRequest) {
    let Some(preset) = builtin_provider_preset(req.name.as_str()) else {
        return;
    };

    if clean_option_string(req.base_url.clone()).is_none() {
        req.base_url = preset.default_base_url.map(|value| value.to_string());
    }

    let Some(auth_hint) = preset.auth_hint else {
        return;
    };
    ...
    if auth_is_unset {
        req.auth_type =
            provider_auth_type_from_hint(auth_hint.method, auth_hint.header_name, auth_hint.prefix);
    }
...
fn provider_default_model_prompt_label(provider_name: &str) -> String {
    let examples: Vec<&str> = builtin_models_for_provider(provider_name)
        .into_iter()
        .take(3)
    ...
    format!("default_model (e.g. {})", examples.join(", "))
}

fn model_provider_prompt_label(model_name: &str, base: &str) -> String {
    let mut providers = BTreeSet::new();
    for candidate in builtin_provider_candidates_for_model(model_name)
        .into_iter()
        .take(6)
    {
...
fn infer_default_base_url(
    namespace: ProviderNamespace,
    provider_name: &str,
    upstream_api: ProviderApi,
) -> Option<String> {
    if let Some(preset) = builtin_provider_preset(provider_name) {
    ...
    if name.contains("yunwu") {
        let url = match upstream_api {
            ProviderApi::GeminiGenerateContent => "https://yunwu.ai/v1beta",
            _ => "https://yunwu.ai/v1",
        };
    ...
    match namespace {
        ProviderNamespace::Google | ProviderNamespace::Gemini => {
...
fn apply_provider_auth_table(
    ...
    match req.auth_type {
        ...
        ProviderAuthType::Command => {
            ...
            if command.is_empty() {
                return Err(DittoError::Config(
                    "auth_type=command requires auth_command".to_string(),
                ));
            }
...
fn apply_provider_capabilities_table(provider_table: &mut Table, req: &ProviderUpsertRequest) {
...
async fn discover_models_for_provider(
    ...
    if base_url.is_empty() {
        return Err(DittoError::Config(
            "provider base_url must not be empty for discover_models".to_string(),
        ));
    }
    ...
    let key = resolve_discovery_key(req)?;
    ...
    match req.auth_type {
        ...
        ProviderAuthType::HttpHeaderEnv => {
            let header = clean_option_string(req.auth_header.clone())
                .unwrap_or_else(|| "Authorization".to_string());
            let auth_value = if let Some(prefix) = normalize_auth_prefix(req.auth_prefix.clone()) {
                format!("{prefix}{key}")
    ...
    let response = request.send().await.map_err(DittoError::Http)?;
    ...
    dedup_sort_strings(&mut models);
    Ok(models)
}

fn resolve_discovery_key(req: &ProviderUpsertRequest) -> Result<String> {
    if let Some(api_key) = clean_option_string(req.discovery_api_key.clone()) {
        return Ok(api_key);
    }

    for key in clean_string_list(req.auth_keys.clone()) {
...
fn parse_openai_models(payload: &JsonValue) -> Vec<String> {
    payload
        .get("data")
...
fn parse_gemini_models(payload: &JsonValue) -> Vec<String> {
...
fn resolve_global_omne_root() -> Result<PathBuf> {
    ...
    let Some(home) = std::env::var_os("HOME").map(PathBuf::from) else {
        return Err(DittoError::Config(
            "cannot resolve global config root: HOME is not set".to_string(),
        ));
    };
...
async fn load_or_create_document(path: &PathBuf) -> Result<DocumentMut> {
    if try_exists(path.as_path()).await {
        ...
        return raw
            .parse::<DocumentMut>()
            .map_err(|err| DittoError::Config(format!("parse {}: {err}", path.display())));
    }
...
fn item_to_json(item: &Item) -> Result<JsonValue> {
    if !item.is_table() {
        return Err(DittoError::Config(
            "item_to_json requires table item".to_string(),
        ));
    }
    let raw = item.to_string();
    let parsed = toml::from_str::<toml::Value>(&raw)
        .map_err(|err| DittoError::Config(format!("parse table item for json: {err}")))?;
    serde_json::to_value(parsed)
        .map_err(|err| DittoError::Config(format!("serialize table item to json: {err}")))
}
