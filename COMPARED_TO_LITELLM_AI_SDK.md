...
- Translation proxy：OpenAI in/out 的 `POST /v1/chat/completions` + `POST /v1/responses`（backend 配置 `provider`；feature `gateway-translation`）
...
- Translation：当前仅覆盖 `POST /v1/chat/completions`/`POST /v1/responses`；其余 OpenAI 端点（embeddings/images/audio/batches 等）的 translation 仍需扩面
