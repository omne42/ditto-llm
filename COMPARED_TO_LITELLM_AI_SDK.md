...
- 平台扩展项：✅ A2A agent gateway（LiteLLM-like `/a2a/*` JSON-RPC 代理）+ ✅ MCP gateway（LiteLLM-like `/mcp*` + `/v1/chat/completions` 的 `tools: [{\"type\":\"mcp\", ...}]`）已支持；仍缺 LiteLLM 那种更细粒度的 MCP 权限/参数白名单等企业策略（可先用 `allowed_tools` + 多实例隔离承接）
