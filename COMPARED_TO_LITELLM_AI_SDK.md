...
- Translation proxy：OpenAI in/out 的 `POST /v1/chat/completions` + `POST /v1/responses` + `POST /v1/embeddings` + `POST /v1/moderations` + `POST /v1/images/generations` + `POST /v1/audio/transcriptions` + `POST /v1/audio/speech`（backend 配置 `provider`；feature `gateway-translation`）
...
- Translation：当前覆盖 `POST /v1/chat/completions`/`POST /v1/responses`/`POST /v1/embeddings`/`POST /v1/moderations`/`POST /v1/images/generations`/`POST /v1/audio/transcriptions`/`POST /v1/audio/speech`；其余 OpenAI 端点（batches 等）的 translation 仍需扩面
