...
#[tokio::test]
async fn openai_compat_proxy_auto_executes_mcp_tool_calls_for_responses_when_require_approval_never()
{
    if ditto_llm::utils::test_support::should_skip_httpmock() {
        return;
    }
...
#[tokio::test]
async fn openai_compat_proxy_auto_executes_mcp_tool_calls_for_responses_via_shim_when_upstream_lacks_responses(
) {
    if ditto_llm::utils::test_support::should_skip_httpmock() {
        return;
    }
    ...
    let openai_chat_initial = openai_upstream.mock(|when, then| {
        when.method(POST)
            .path("/v1/chat/completions")
            .header("authorization", "Bearer sk-test")
            .header("x-request-id", "req-1-mcp0")
    ...
    let openai_chat_follow_up = openai_upstream.mock(|when, then| {
        when.method(POST)
            .path("/v1/chat/completions")
            .header("authorization", "Bearer sk-test")
            .header("x-request-id", "req-1")
