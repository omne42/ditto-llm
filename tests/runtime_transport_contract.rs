...
use ditto_llm::config::{ProviderAuth, ProviderConfig};
use ditto_llm::contracts::{CapabilityKind, OperationKind};
#[cfg(all(
    feature = "realtime",
    any(feature = "provider-openai", feature = "openai")
))]
use ditto_llm::runtime::RuntimeTransportBaseUrlRewrite;
...
fn runtime_transport_plan_exposes_openai_compatible_http_shape() {
    ...
    let plan = ditto_llm::runtime::plan_builtin_runtime_transport(
